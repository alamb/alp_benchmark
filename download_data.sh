...
# Downloads the CWI ALP corpus into ./data  and extracts the datasets.
#
# If the datasets are already present nothing is
# downloaded.
#
# Set ALP_DATASET_DIR to use a different dataset
# directory and ALP_DOWNLOAD_DIR to store the archive elsewhere.
...
DOWNLOAD_DIR="${ALP_DOWNLOAD_DIR:-${DATA_DIR}}"
...
download_datasets() {
  ...
  if [[ -f "$ARCHIVE" ]] && [[ "$(sha256_file "$ARCHIVE")" == "$ARCHIVE_SHA256" ]]; then
    echo "Using the cached archive in ${ARCHIVE}" >&2
  else
